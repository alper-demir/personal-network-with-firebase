...
export const createPost = async (req, res) => {
    const { userId, content, contentUrl, contentFileType } = req.body;

    try {
        const newPost = await Post.create({ userId, content, contentUrl, contentFileType });
        const postId = newPost._id;
        const updatedUser = await User.findOneAndUpdate({ _id: userId }, { $push: { posts: postId } }, { new: true });
...
export const getOnePost = async (req, res) => {
    ...
    try {
        const post = await Post.findById(postId).populate("userId", { username: 1, profileUrl: 1, firstName: 1, lastName: 1, uid: 1 }) // owner of post
            .populate({
                path: "comments",
                populate: {
                    path: "userId",
                    select: { username: 1, profileUrl: 1, firstName: 1, lastName: 1, uid: 1 } // selection of needed fields
                }
            })
