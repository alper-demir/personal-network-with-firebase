...
import calculateTimeAgo from "../helpers/calculateTimeAgo.js"
...
const PostDetail = () => {
    ...
    const checkPostLikedStatus = async () => {
        ...
        try {
            const liked = await axios.post(`${URL}/check-post-like-status`, { userId, postId });
    ...
    const getPostData = async () => {
        try {
            const postData = await axios.get(`${URL}/post/${postId}`);
            if (postData) {
    ...
    const likePost = async () => {
        const likePost = await axios.post(`${URL}/like-post`, { userId, postId });
    ...
    const deleteComment = async (id) => {
        ...
        if (deleteConf) {
            try {
                const deleteComment = await axios.post(`${URL}/delete-comment/${id}`, { postId });
    ...
    const shareComment = async () => {
        if (comment.length > 0) {
            ...
            try {
                const createComment = await axios.post(`${URL}/create-comment`, { userId, postId, content: comment });
                if (createComment) {
    ...
    const handleEditPost = async () => {
        try {
            const editPost = await axios.put(`${URL}/update-post/${postId}`, { content });
    ...
    const handleDeletePost = async () => {
        ...
        if (deleteConfirm) {
            try {
                const deletePost = await axios.delete(`${URL}/delete-post/${postId}`);
    ...
    const handleEditComment = async () => {
        try {
            const editComment = await axios.post(`${URL}/edit-comment/${editedCommentId}`, { content: editCommentText });
                            ...
                            <div className='flex flex-col justify-center -mt-1 gap-y-1 dark:text-white p-3'>
                                    ...
                                    <div className="flex gap-x-1 items-center">
                                        <div>
                                            <Link to={`/profile/${post.userId?.username}`}>
                                                <img src={`${post.contentUrl}`} alt="img" className='min-w-[36px] h-9 w-9 object-cover rounded-full' />
                                            </Link>
                                        </div>
                                ...
                                <div className="mt-1">
                                    {
                                        post.contentFileType === "video" ? (
                                            <video controls src={post.contentUrl} className="rounded-md object-cover w-full" />
                                        ) : (
                                            <img src={post.contentUrl} className="rounded-md object-cover w-full" alt="img" />
                                        )
                                    }
                                </div>
                                ...
                                <div className='flex gap-1 -ml-2'>
                                    <div onClick={likePost} className='hover:bg-[#F5F5F5] dark:hover:bg-[#1E1E1E] p-2 rounded-full transition-colors duration-200 cursor-pointer'>
                                        <AiOutlineLike className={`text-2xl ${currentUserLikedPost ? "text-indigo-500" : ""}`} title='Like' />
                                    </div>
                            ...
                            <div className="overflow-y-auto max-h-96">
                                {
                                    ...
                                    postComments.map((comment, index) => (
                                        <div key={index} className="border-t p-3 dark:text-white break-words dark:border-opacity-20 border-gray-200">
                                            <div className="flex justify-between items-center">

                                                <div className="flex gap-x-1 items-center">
                                                    <div>
                                                        <Link to={`/profile/${comment.userId.username}`}>
                                                            <img src={`${comment.userId.profileUrl}`} alt="img" className='min-w-[36px] h-9 w-9 object-cover rounded-full' />
                                                        </Link>
                                                    </div>
                                                    ...
                                                    <div className='flex flex-col gap-x-1 font-semibold max-sm:flex-col'>
                                                        ...
                                                        <div className="text-sm max-sm:text-xs">
                                                            <Link to={`/profile/${comment.userId.username}`}>@{comment.userId.username}</Link>
                                                        </div>
                                                    </div>
            ...
            >
                <Box className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-1/5 max-xl:w-2/5 max-md:w-3/5 p-4 bg-white dark:bg-[#101010] dark:text-white max-h-80 overflow-y-auto rounded-md">
                    ...
                    <Typography id="modal-modal-description" sx={{ mt: 2 }} >
                        {
                            postLikes && postLikes.map((postLike, index) => (
                                <div key={index} className="flex gap-x-2 items-center my-1 text-[15px] max-sm:text-xs max-lg:text-sm">
                                    <div>
                ...
                <Box className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-1/4 max-xl:w-3/5 max-md:w-4/5 p-4 bg-white dark:bg-[#101010] dark:text-white max-h-80 overflow-y-auto rounded-md">
                    ...
                    <Typography id="modal-modal-description" sx={{ mt: 2 }} >
                        ...
                        <div className="flex justify-end bg-white dark:bg-transparent my-3">
                            <button type="button" className="focus:outline-none text-white bg-yellow-400 hover:bg-yellow-500 focus:ring-4 focus:ring-yellow-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:focus:ring-yellow-900" onClick={handleEditPost}>Edit Post</button>
                        </div>
                        <div className="flex justify-end bg-white dark:bg-transparent my-3">
                            <button type="button" className="focus:outline-none text-white bg-red-500 hover:bg-red-600 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:focus:ring-red-900" onClick={handleDeletePost}>Delete Post</button>
                        </div>
                    </Typography>
                </Box>
            </Modal>
            ...
            >
                <Box className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-1/5 max-xl:w-2/5 max-md:w-3/5 p-4 bg-white dark:bg-[#101010] dark:text-white max-h-80 overflow-y-auto rounded-md">
                    ...
                    <Typography id="modal-modal-description" sx={{ mt: 2 }} >
                        ...
                        <div className="flex justify-end bg-white dark:bg-transparent my-3">
                            <button type="button" className="focus:outline-none text-white bg-yellow-400 hover:bg-yellow-500 focus:ring-4 focus:ring-yellow-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:focus:ring-yellow-900" onClick={handleEditComment}>Edit Comment</button>
                        </div>
                    </Typography>
                </Box>
            </Modal>
